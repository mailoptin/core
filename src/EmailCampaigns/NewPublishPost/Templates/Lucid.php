...
class Lucid extends AbstractTemplate
{
    ...
    public function _post_title_feature_img()
    {
        ...
        if ($content_remove_post_link == false) : ?>
            ...
            <a href="{{post.url}}">
                <h1 class="mo-content-title-font-size mo-content-headline-color">
                    {{post.title}}</h1>
            </a>
            {{post.meta}}
            <a href="{{post.url}}">
                <img class="mo-imgix" src="{{post.feature.image}}" alt="{{post.feature.image.alt}}" width="500" style="max-width:500px;height:auto;display:block;margin:0 auto;">
            </a>
        <?php endif;
    ...
    protected function get_footer_html()
    {
        ...
        <?php
        return apply_filters('mo_npp_lucid_email_template_footer_html', ob_get_clean(), $this);
    }
